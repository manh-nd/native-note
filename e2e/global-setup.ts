...
export default async function globalSetup() {
  ...
  await sql.begin(async (tx) => {
    ...
    await tx`delete from skill_versions where skill_id in (select skills.id from skills join pages on pages.id = skills.page_id where pages.workspace_id in (${DESKTOP_WORKSPACE_ID}, ${MOBILE_WORKSPACE_ID}) and pages.id not in (${DESKTOP_PAGE_ID}, ${MOBILE_PAGE_ID}))`;
    await tx`delete from skills where page_id in (select id from pages where workspace_id in (${DESKTOP_WORKSPACE_ID}, ${MOBILE_WORKSPACE_ID}) and id not in (${DESKTOP_PAGE_ID}, ${MOBILE_PAGE_ID}))`;
