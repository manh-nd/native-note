...
export default async function globalSetup() {
  ...
  await sql.begin(async (tx) => {
    ...
    await tx`insert into pages (id, workspace_id, title, content, plain_text, position, version) values (${DESKTOP_PAGE_ID}, ${DESKTOP_WORKSPACE_ID}, 'Block editor test', ${tx.json(content)}, 'First block\nSecond block\nBullet item\nNumber item', 0, 1) on conflict (id) do update set title = excluded.title, content = excluded.content, plain_text = excluded.plain_text, parent_id = null, position = 0, version = 1, deleted_at = null`;
    await tx`insert into pages (id, workspace_id, title, content, plain_text, position, version) values (${MOBILE_PAGE_ID}, ${MOBILE_WORKSPACE_ID}, 'Block editor test', ${tx.json(content)}, 'First block\nSecond block\nBullet item\nNumber item', 0, 1) on conflict (id) do update set title = excluded.title, content = excluded.content, plain_text = excluded.plain_text, parent_id = null, position = 0, version = 1, deleted_at = null`;
