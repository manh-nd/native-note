...
type Props = {
  ...
  pages: PageRow[];
  user: User;
  skillsByPageId: Record<string, SkillRow>;
  onAddPage: (markAsSkill: boolean) => void;
  ...
  onMarkSkill: (pageId: string) => void;
  onUnmarkSkill: (pageId: string) => void;
};
...
export function buildPageTree(pageList: PageRow[]) {
  const nodes = new Map<string, PageNode>(
    pageList.map((page) => [page.id, { ...page, children: [] as PageNode[] }])
  );
...
function PageItem({ node, ...props }: PageItemProps) {
  ...
  const siblings = props.pages.filter(
    (page) => page.parentId === node.parentId
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          render={
            <SidebarMenuAction
              showOnHover
              aria-label={`Tùy chọn cho ${node.title}`}
            />
          }
        >
          <MoreHorizontal />
        ...
        <DropdownMenuContent side="right" align="start" className="w-52">
          <DropdownMenuGroup>
            ...
            <DropdownMenuItem onClick={() => props.onRenamePage(node.id)}>
              <Pencil />
              Đổi tên
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={async () => {
                await navigator.clipboard.writeText(
                  `${window.location.origin}/workspace?page=${node.id}`
                );
            ...
            {props.skill ? (
              <DropdownMenuItem onClick={() => props.onUnmarkSkill(node.id)}>
                <Sparkles />
                Bỏ đánh dấu Skill
              </DropdownMenuItem>
            ) : (
              <DropdownMenuItem onClick={() => props.onMarkSkill(node.id)}>
                <Sparkles />
                Đánh dấu là Skill
              </DropdownMenuItem>
            )}
          ...
          <DropdownMenuGroup>
            <DropdownMenuItem
              ...
              onClick={() => props.onMovePage(node.id, "up")}
            >
              <MoveUp />
              Di chuyển lên
            </DropdownMenuItem>
            <DropdownMenuItem
              ...
              onClick={() => props.onMovePage(node.id, "down")}
            >
              <MoveDown />
              Di chuyển xuống
            </DropdownMenuItem>
            <DropdownMenuItem
              ...
              onClick={() => props.onMovePage(node.id, "indent")}
            >
              <IndentIncrease />
              Lồng vào trang trước
            </DropdownMenuItem>
            <DropdownMenuItem
              disabled={!node.parentId}
              onClick={() => props.onMovePage(node.id, "outdent")}
            >
              <IndentDecrease />
              Đưa ra cấp gốc
            </DropdownMenuItem>
          ...
          <DropdownMenuGroup>
            ...
            >
              <Trash2 />
              Xóa{subtreeCount > 1 ? ` ${subtreeCount} trang` : " trang"}
            </DropdownMenuItem>
          </DropdownMenuGroup>
        </DropdownMenuContent>
      </DropdownMenu>
          ...
          <SidebarMenuSub>
            {node.children.map((child) => (
              <PageItem
                ...
                node={child}
                {...props}
                skill={props.skillsByPageId[child.id]}
              />
            ))}
          </SidebarMenuSub>
        </CollapsibleContent>
...
export function WorkspaceSidebar({
  ...
  const deleteCount = deleteTarget
    ? pageSubtreeIds(pageList, deleteTarget.id).length
    : 0;
      ...
      <Sidebar collapsible="offcanvas">
        ...
        <SidebarContent>
          ...
          <SidebarGroup>
            ...
            >
              <Plus />
              <span className="sr-only">Tạo trang mới</span>
            </SidebarGroupAction>
            <SidebarGroupContent>
              <SidebarMenu>
                {tree.map((node) => (
