...
export function normalizeBlockTarget(
  ...
  for (let depth = resolved.depth; depth > 0; depth -= 1) {
    ...
    if (
      ...
      parent.attrs?.[BLOCK_ID_ATTRIBUTE]
    ) {
      return {
        id: parent.attrs[BLOCK_ID_ATTRIBUTE],
        node: parent,
...
export function insertParagraphAfter(editor: Editor, target: BlockTarget) {
  ...
  editor
    .chain()
    .focus()
    .insertContentAt(insertionPos, { type: "paragraph", attrs: { blockId } })
...
export function duplicateBlock(editor: Editor, blockId: string) {
  ...
  return editor
    .chain()
    .focus()
    .insertContentAt(target.pos + target.node.nodeSize, json)
    .run();
}
...
export function deleteBlock(editor: Editor, blockId: string) {
  ...
  if (
    (resolved.parent.type.name === "bulletList" ||
      resolved.parent.type.name === "orderedList" ||
      resolved.parent.type.name === "taskList") &&
...
export function moveBlock(
  ...
  if (direction === "up") {
    ...
    tr.delete(target.pos, target.pos + target.node.nodeSize).insert(
      target.pos - previous.nodeSize,
      target.node
    );
  } else {
    const next = parent.child(index + 1);
    tr.delete(target.pos, target.pos + target.node.nodeSize).insert(
      target.pos + next.nodeSize,
      target.node
    );
  }
  editor.view.dispatch(tr.scrollIntoView());
...
export function moveBlockRelative(
  ...
  if (insertion >= source.pos && insertion <= source.pos + source.node.nodeSize)
    return false;
  const tr = editor.state.tr.delete(
    source.pos,
    source.pos + source.node.nodeSize
  );
  const mappedInsertion = tr.mapping.map(
    insertion,
    insertion > source.pos ? -1 : 1
  );
  try {
...
export function turnBlockInto(
  ...
  if (
    ...
    target.node.type.name === "taskItem"
  )
    chain = chain.liftListItem(target.node.type.name);
  if (kind === "paragraph") return chain.setParagraph().run();
  if (kind === "heading1" || kind === "heading2" || kind === "heading3")
    return chain.setHeading({ level: Number(kind.at(-1)) as 1 | 2 | 3 }).run();
...
export function isAiBlockResultStale(
  ...
  currentContentRevision: number | undefined,
