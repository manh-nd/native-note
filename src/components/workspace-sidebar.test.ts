...
describe("workspace page tree", () => {
  it("sorts siblings and nests descendants", () => {
    const pages = [page("second", null, 2), page("child", "first", 0), page("first", null, 1)];
    const tree = buildPageTree(pages);

    expect(tree.map((node) => node.id)).toEqual(["first", "second"]);
    expect(tree[0].children.map((node) => node.id)).toEqual(["child"]);
    expect(pageSubtreeIds(pages, "first")).toEqual(expect.arrayContaining(["first", "child"]));
  });

  it("promotes orphaned or cyclic pages to roots instead of recursing forever", () => {
    const pages = [page("orphan", "missing", 0), page("a", "b", 1), page("b", "a", 2)];
    const tree = buildPageTree(pages);
    const visible = new Set(tree.flatMap((node) => [node.id, ...node.children.map((child) => child.id)]));
