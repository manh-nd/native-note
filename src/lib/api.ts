...
export async function requireUserId() {
  ...
  if (!session?.user?.id) throw new ApiError(401, "Bạn cần đăng nhập để tiếp tục.", "UNAUTHORIZED");
...
export function apiError(error: unknown) {
  ...
  console.error(error instanceof Error ? { name: error.name, message: error.message } : "Unknown API error");
...
export async function parseJson<T>(request: Request, schema: { parse(value: unknown): T }): Promise<T> {
  ...
  if (length > 200_000) throw new ApiError(413, "Nội dung vượt quá giới hạn cho phép.", "PAYLOAD_TOO_LARGE");
  try {
