...
export const findingStatus = pgEnum("finding_status", ["pending", "applied", "dismissed", "saved", "stale"]);
export const learningStatus = pgEnum("learning_status", ["active", "mastered", "archived"]);
export const practiceVerdict = pgEnum("practice_verdict", ["correct", "partially_correct", "incorrect"]);
...
export const pages = pgTable("pages", {
  ...
  content: jsonb("content").$type<JSONContent>().notNull().default({ type: "doc", content: [{ type: "paragraph" }] }),
...
}, (table) => [index("pages_workspace_parent_idx").on(table.workspaceId, table.parentId, table.position)]);
...
export const learningItems = pgTable("learning_items", {
  id: uuid("id").defaultRandom().primaryKey(),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  findingId: uuid("finding_id").unique().references(() => findings.id, { onDelete: "set null" }),
  category: findingCategory("category").notNull(),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("learning_due_idx").on(table.userId, table.status, table.dueAt)]);
...
export const practiceAttempts = pgTable("practice_attempts", {
  ...
  itemId: uuid("item_id").notNull().references(() => learningItems.id, { onDelete: "cascade" }),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("attempt_context_idx").on(table.itemId, table.contextFingerprint)]);
